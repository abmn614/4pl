...
if (!empty($_POST['old_psw'])) {
    if (!empty($_POST['new_psw']) && !empty($_POST['new_repsw'])) {
        ...
        if ($old_password === $userinfo[0]['password']) { // 旧密码正确
            if ($new_password === $new_repassword) { // 新密码匹配
                ...
                if (isset($_POST['submit'])) { // 判断是否提交了表单
                    ...
                    $log->insert($insert_log);
                        ...
                        echo "<script>alert('{$lang['密码修改成功']}！');history.go(-1);</script>";
                    }else{
                        echo "<script>alert('{$lang['密码未作出任何修改']}！');history.go(-1);</script>";
                    }
                }
            }else{
                echo "<script>alert('{$lang['两次输入的新密码不一致']}！');history.go(-1);</script>";
            }
        }else{
            echo "<br><br><br>";
            ...
            echo "<br><br><br>";
            var_dump($userinfo[0]['password']) ;
            echo "<script>alert('{$lang['旧密码不正确']}！');history.go(-1);</script>";
        }
    }else{
        echo "<script>alert('{$lang['请设置新密码']}！');history.go(-1);</script>";
    }
}else{
    // 更新到数据库
    if (isset($_POST['submit'])) {
        ...
        if ($flag) {
            ...
            echo "<script>alert('{$lang['资料修改成功']}！');history.go(-1);</script>";
        }else{
            echo "<script>alert('{$lang['资料未作出任何修改']}！');history.go(-1);</script>";
        }
    }
}
...
<div class="container reg">
    <form action="alter.php" method="post" class="form-horizontal">
        ...
        <div class="control-group">
            <label for="old-password" class="control-label"><?=$lang['旧密码']?></label>
            <div class="controls">
                <input type="password" id="old-passowrd" name="old_psw" placeholder="<?=$lang['旧密码']?>">
                <p class="text-warning"><?=$lang['如果需要修改密码则填此项和下面两项，否则跳过。']?></p>
            </div>
        </div>
        <div class="control-group">
            <label for="new-password" class="control-label"><?=$lang['新密码']?></label>
            <div class="controls">
                <input type="password" id="new-passowrd" name="new_psw" placeholder="<?=$lang['新密码']?>">
            </div>
        </div>
        <div class="control-group">
            <label for="re-new-password" class="control-label"><?=$lang['重复新密码']?></label>
            <div class="controls">
                <input type="password" id="re-new-passowrd" name="new_repsw" placeholder="<?=$lang['重复新密码']?>">
            </div>
        </div>
        <div class="control-group">
            <label for="tel" class="control-label"><?=$lang['座机']?></label>
            <div class="controls">
                <input type="text" id="tel" placeholder="<?=$lang['座机']?>" name="tel" value="<?=$userinfo[0]['tel']?>" required>
            </div>
        </div>
        <div class="control-group">
            <label for="mobile" class="control-label"><?=$lang['手机']?></label>
            <div class="controls">
                <input type="text" id="mobile" placeholder="<?=$lang['手机']?>" name="cellphone" value="<?=$userinfo[0]['cellphone']?>" required>
            </div>
        </div>
        <div class="control-group">
            <label for="qq" class="control-label">QQ</label>
            <div class="controls">
                <input type="text" id="qq" placeholder="QQ" name="qq" value="<?=$userinfo[0]['qq']?>" required>
            </div>
        </div>
        <div class="control-group">
            <label for="email" class="control-label"><?=$lang['邮箱']?></label>
            <div class="controls">
                <input type="email" id="email" placeholder="<?=$lang['邮箱']?>" name="email" value="<?=$userinfo[0]['email']?>" required>
            </div>
        </div>
        <div class="control-group">
            <label for="adress" class="control-label"><?=$lang['地址']?></label>
            <div class="controls">
                <input type="text" id="adress" placeholder="<?=$lang['地址']?>" name="adress" value="<?=$userinfo[0]['adress']?>" required>
            </div>
        </div>
        <div class="control-group">
            <label for="fahuoren" class="control-label"><?=$lang['发货人']?></label>
            <div class="controls">
                <input type="text" id="fahuoren" placeholder="<?=$lang['发货人']?>" name="shipper" value="<?=$userinfo[0]['shipper']?>" required>
            </div>
        </div>
        <div class="control-group">
            <label for="zhiwei" class="control-label"><?=$lang['发货人职位']?></label>
            <div class="controls">
                <input type="text" id="zhiwei" placeholder="<?=$lang['发货人职位']?>" name="shipper_title" value="<?=$userinfo[0]['shipper_title']?>" required>
            </div>
        </div>
        <div class="control-group">
